...
REM This script is a sample of windows script that can be used to synchronize an existing elpaaso database with liquibase changelog
REM It applies on elpaaso  1.1 release
REM
...
java  -jar %liquibase_jar% --classpath="%postgresql_jar%" --changeLogFile="./paas-db-changelog-1.0-full.xml" --url="%db_url%" --username="%db_username%" --password="%db_password%"  changelogSync
java  -jar %liquibase_jar% --classpath="%postgresql_jar%" --changeLogFile="./paas-db-changelog-1.1-delta.xml" --url="%db_url%" --username="%db_username%" --password="%db_password%" changelogSync
